class World {
    constructor(graph, 
        ...
        spacing = 50,
        treeSize = 160 //defining sieze of the tree
    ) 
    {
        this.graph = graph;
    ...
    #generateLaneGuides() { //privates methods
        const tmpEnvelopes = [];
        for (const seg of this.graph.segments) {
            tmpEnvelopes.push(
                new Envelope(
    ...
    #generateTrees() {
        ...
        const trees = [];
        let tryCount = 0; //making sure every area has maximum trees
        while (tryCount < 100) { //generating trees where the building cannot fit
            const p = new Point(
            ...
            //filter trees generated at long points 
            if (keep) {
    ...
    #generateBuildings(){
        ...
        //making rectangular shapes
        const bases = [];
        for (const seg of supports) {
            bases.push(new Envelope(seg, this.buildingWidth).poly);
        }

        //removing the interesection of bases and gapss
        const eps = 0.001;
