class Tree {
    ...
    #generateLevel(point, size) {
        ...
        for (let a =0; a < Math.PI * 2; a += Math.PI / 16) {
            const kindOfRandom = Math.cos(((a + this.center.x) * size) % 17) ** 2; //making each level of the tree diff from the other
    ...
    draw(ctx, viewPoint) {
        ...
        for (let level = 0; level < levelCount; level++) {
            ...
            const poly = this.#generateLevel(point, size); //interpolation
